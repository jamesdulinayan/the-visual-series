...
loadAn.fromTo('.main-menu > li > a',1,{y:'100%'},{y:'0%',stagger:.1,ease:'power2.out',onComplete:function(){gsap.set('.main-menu > li',{clearProps:'all'})}},3)}else{loadAn.to('.toggle-line',1,{width:50,ease:'power2.out',stagger:.3},3)}
...
gsap.to(headerWrapper,{top:'40%',delay:.5,duration:.75,ease:'power2.out'})},onEnter:function(){gsap.set(siteHeader,{position:'absolute',top:stickyStart})
...
gsap.set(headerWrapper,{top:'40%'})},onUpdate:function(self,direction,progress){if(self.direction==-1){gsap.to(siteHeader,{y:'0%',})}else{gsap.to(siteHeader,{y:'-100%',})}},onLeaveBack:function(self){self.kill();}})},onEnterBack:function(){gsap.set(siteHeader,{position:'fixed',top:0})}})}else{gsap.set(siteHeader,{position:'absolute'})
...
$('.tit_word, .project-other h5, .summ_line, .project-cat').wrapInner("<span></span>");if(projectHeader.hasClass('style_1')){let pphAnim=gsap.timeline();pphAnim.to('.tt-char',1.5,{y:'0%',stagger:0.02,ease:'power3.out'},0)
...
InertiaPlugin.track(apWrapper,"x");var apcDrag=Draggable.create(apWrapper,{type:"x",bounds:$this,autoScroll:true,inertia:true,edgeResistance:0.75,dragResistance:0.4,throwProps:true,});if(willAnim==true){gsap.fromTo(apWrapper,{x:-5100},{x:'0%',duration:4,ease:'power2.inOut',scrollTrigger:{trigger:$this,}})
...
service.find('.post-title, .service-toggle').on('click',function(e){e.stopPropagation();let parent=$(this).closest('.service'),content=parent.children('.service-wrap'),contentInner=content.children('.service-cont'),contHeight=parent.data('height');if(parent.hasClass('active')){parent.removeClass('active');gsap.to(content,0.6,{height:0,ease:'power1.out',delay:0.1})
gsap.to(contentInner,0.8,{opacity:0,ease:'power1.out'})}else{service.removeClass('active')
parent.addClass('active');let other=service.not('.active'),otherContent=other.children('.service-wrap'),otherInner=other.children('.service-cont');gsap.to(content,0.6,{height:contHeight,ease:'power1.out'})
gsap.to(contentInner,0.8,{opacity:1,delay:0.1,ease:'power1.out'})
gsap.to(otherContent,0.6,{height:0,ease:'power1.out',},0.1)
...
if(!mobileQuery.matches){$this.on('mousemove',function(e){let mouseLeft=e.pageX,mouseTop=e.pageY,names=$this.find('.aph-name')
gsap.to(names,{x:-mouseLeft/10,duration:.6,});gsap.to('.aph-image',{x:-mouseLeft/20,duration:.6,});})}}},.5)
...
aliothServicesS2();aliothTestimonials();aliothImageCarousel();aliothHeading();aliothClients();aliothAwards();alitohServicesS1();aliothPersonalHead();aliothForms();alitohNumberCt();aliothScrollNotice();aliothProductsCarousel();aliothSingleProduct();aliothRecentWorks();}
...
resolve()}})}},0)})},enter(){return new Promise(function(resolve,reject){gsap.to('.trans-image',{height:'100vh',duration:1,ease:'power2.out',onComplete:function(){resolve();$('.trans-image').remove();$('body').removeClass('loading');enableScroll();}})})}},{name:'scwall-image-trans-half',from:{namespace:['sc-wall']},to:{namespace:['pph1']},leave(){return new Promise(function(resolve,reject){disableScroll();var project=$('.wall-project.hovered').data('image'),img=$(project).find('img'),imgURL=img.attr('src');var scImageTrans=gsap.timeline();scImageTrans.to('.wall-projects-top',2,{x:'100%',stagger:0.05,ease:'power2.inOut',onStart:function(){$('body').addClass('loading');$('body').append('<div class="trans-image"><div class="trans-image-wrap"><img src="'+imgURL+'"></div></div>');const mobileQuery=window.matchMedia('(max-width: 450px)')
if(mobileQuery.matches){gsap.set('.trans-image',{width:'84%',height:'70%',top:'50%',left:'50%',y:'-50%',x:'-50%',zIndex:-1})
$('.wall-images').remove();}else{gsap.set('.trans-image',{width:'30%',height:'70%',top:'50%',left:'50%',y:'-50%',x:'-50%',zIndex:-1})
$('.wall-images').remove();}}},0)
scImageTrans.to('.wall-projects-bottom',2,{x:'-100%',stagger:0.05,ease:'power2.inOut',},0)
...
gsap.set(trans,{visibility:'hidden'})}},0)}}]});

if(history.scrollRestoration){history.scrollRestoration='manual';};}(jQuery));